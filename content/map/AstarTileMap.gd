...
func _unhandled_input(event):
	if event is InputEventMouseButton:
		if event.button_index == BUTTON_LEFT and event.pressed:
			var pos = get_global_mouse_position() - get_parent().position
			get_parent().get_node("ColorRect").position = pos
			var clicked_cell := world_to_map(pos)
			var point = astar.get_closest_point(pos - cell_size/2)
			printt(point, clicked_cell)
			path = astar.get_id_path(1104, point)
...
func create_pathfinding_points() -> void:
	...
	for cell_position in used_cell_positions:
		var density = get_parent().get_hardnessv(cell_position)

		var world_cell = map_to_world(cell_position, true)
		var id = get_point(world_cell)

		if density >= 0:
			astar.add_point(id, world_cell, density)
